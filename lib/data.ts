...
export const projectsData = [
  ...
  },
  {
    title: "Sharebby",
    description:
      "Sharebby는 Share + hobby의 줄임말로 취미를 공유하고, 새로운 사람들과 함께 취미 활동을 즐길 수 있는 크로스플랫폼 앱입니다. 일상에 활력을 더하고, 새로운 인연을 만들어가는 여정을 시작해보세요!",
    features: [
      {
        title: "취미 공유",
    ...
    troubleShooting: [
      {
        ...
        description:
          "로그인(’/login’), 회원가입(’/create-account’) 페이지 배경으로 사용하는 애니메이션이 들어간 <Sparkles/> 컴포넌트 크기가 너무 커서 초기 로딩이 오래 걸리는 문제가 발생했습니다.",
        solution:
          "React lazy를 사용해 해당 컴포넌트를 동적으로 불러오게 처리하고, Suspense 컴포넌트로 로딩 상태를 화면에 표시해주었습니다.",
        learned:
          "이 문제를 해결하면서 React-query의 캐시 관리와 invalidateQueries의 역할을 깊이 이해하게 되었습니다. 특히, 서버 상태를 효율적으로 동기화하고, 사용자 경험을 향상시키기 위한 비동기 처리 전략의 중요성을 배웠습니다. 또한, optimistic update를 통해 사용자 인터페이스를 더 신속하게 업데이트하고, 비동기 작업이 실패할 경우 롤백하는 방법을 익혔습니다. 이러한 기술들을 통해 사용자 경험을 개선하는 동시에 서버와 클라이언트 간의 상태 동기화 문제를 보다 효과적으로 관리할 수 있다는 것을 깨달았습니다.",
      },
      {
